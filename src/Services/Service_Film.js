...
const S_Add_Film = (data) =>{
    return new Promise(async (resolve, reject)=>{
        try{
            const{Film_Id, Category_Id, Name, Director, Start_Date, End_Date, Time, Des, Poster, Member, Rated} = data;

            const check_id_category = await Query("SELECT Category_Id FROM Category WHERE Category_Id=?",[Category_Id]);
            if(check_id_category.length==0){
                return resolve({ status: 404, message: "Category_Id not found" });
            }

            const check_id_film = await Query("SELECT Film_Id FROM Film WHERE Film_Id=?",[Film_Id]);
            if(check_id_film.length>0){
                return resolve({ status: 404, message: "Film_Id is ready" });
            }

            const check_name_film = await Query("SELECT Name FROM Film WHERE Name=?",[Name]);
            ...
            const sql = "INSERT INTO Film (Film_Id, Category_Id, Name, Director, Start_Date, End_Date, Time, Des, Poster, Member, Rated) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            const result = await Query(sql, [Film_Id, Category_Id, Name, Director, Start_Date, End_Date, Time, Des, Poster, Member, Rated]);
...
const S_Delete_Film = (data) =>{
    return new Promise(async (resolve, reject)=>{
        try{
            ...
            const check_id = await Query("SELECT Film_Id FROM Film WHERE Film_Id=?",[Film_Id]);
            ...
            if (currentDate >= startDate && currentDate <= endDate || currentDate < startDate) {
                return resolve({ status: 400, data: "The film is still being shown or about to be shown and cannot be deleted" });
            }
...
module.exports = {S_Get_Film,S_Get_Film_Details, S_Add_Film, S_Delete_Film, S_Edit_Film};